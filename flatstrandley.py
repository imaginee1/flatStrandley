...
def _outline_runs(pts, idxs, oc_of, ow, prims, cap="round"):
    """Emit the side outline split into runs of one colour, so an outline can be
    faded over part of an element (idxs/pts are aligned). Adjacent runs share a
    boundary point so there is no gap. With a uniform colour this is one stroke,
    identical to the previous single _stroke."""
    n = len(pts)
    if n < 2:
        return
    bounds = [0]
    for k in range(1, n):
        if oc_of(idxs[k]) != oc_of(idxs[k - 1]):
            bounds.append(k)
    bounds.append(n - 1)
    for bi in range(len(bounds) - 1):
        ...
        sub = pts[a:b + 1]
        if len(sub) >= 2:
            prims.append(_stroke(sub, oc_of(idxs[a]), ow, cap))
...
def _emit_sheet(prims, samples, nrm, color_of, cfg, direction, patch, clip,
    ...
    if not draw_arrow:
        ...
        _outline_runs(left, order, oc_of, ow, prims)
        _outline_runs(right, order, oc_of, ow, prims)
        if draw_flatN:
            prims.append(_stroke([left[0], right[0]], oc_of(order[0]), ow, "round"))
        if not patch:
            prims.append(_stroke([left[-1], right[-1]], oc_of(order[-1]), ow, "round"))
        return
    ...
    _outline_runs(bl, body, oc_of, ow, prims)
    _outline_runs(br, body, oc_of, ow, prims)
    if draw_flatN:
        prims.append(_stroke([left[0], right[0]], oc_of(order[0]), ow, "round"))  # N flat end
    ...
    prims.append(_stroke([bl[-1], barbL, tip, barbR, br[-1]], oc_of(end), ow, "round"))
...
def strand_prims(path, types, colors, dirs, cfg, clip=None, patch=False,
    ...
    hi_range=(a, b) keeps edges [a, b) at full colour and fades the rest (used to
    highlight a linear-view stretch while dimming the rest of the same chain).
    """
    if len(path) < 2:
        return []
    ...
    for el in els:
        ...
        def color_of(j, _e0=e0, _ne=ne_el, _fr=fracs):
            ...
            if cfg.get("loops_black") and types[le] == "loop":
                base = "#000000"
            elif color_mode == "ss" and ss_colors:
                base = ss_colors.get(types[le], colors[le])
            else:
                base = colors[le]
            if hi_range is not None and not (hi_range[0] <= le < hi_range[1]):
                return _fade(base)
            return base

        def oc_of(j, _e0=e0, _ne=ne_el, _fr=fracs):
            base = cfg["outline"]
            if hi_range is None:
                return base
            le = _e0 + min(_ne - 1, max(0, int(_fr[j] * _ne)))
            if hi_range[0] <= le < hi_range[1]:
                return base
            return _fade(base)
...
def cross_key(lst):
    """Identity of a crossing: the set of participating (strand, vertex) pairs.
    This is what an over/under choice is stored against, so the choice depends
    only on *which* strands cross (a strand's self-crossings and crossings with
    other strands) and never on the crossing's location. It is therefore immune
    to incidental overlap from a nearby paste and survives moves unchanged."""
    return frozenset((sid, vi) for sid, vi, _ in lst)
...
def crossing_tops(crossings, overrides):
    ...
    for p, lst in crossings.items():
        ...
        top = overrides.get(cross_key(lst))
...
def crossing_patches(strands, crossings, overrides, cfg, ss_colors, color_mode,
    ...
    prims = []
    groups = []                         # (sid, vidx, prims) per crossing
    for p in crossings:
        sid, vidx = tops[p]
        s = smap.get(sid)
        if not s:
        ...
        if target["type"] == "sheet":
            ...
            if vidx == e0:
                ...
                dec = "C" if d > 0 else "N"
        pr = strand_prims(s.path, s.types, s.colors, s.dirs, cfg,
                          clip=clip, patch=True, only_edge=edge,
                          decorate_end=dec,
                          ss_colors=ss_colors, color_mode=color_mode)
        prims += pr
        groups.append((sid, vidx, pr))
    return groups if grouped else prims
...
def _fade(col, t=0.74, bg=(244, 244, 246)):
    """Mix a hex colour toward the panel background by fraction t (dimming)."""
    col = (col or "#888888").lstrip("#")
    if len(col) == 3:
...
def _fade_prims(prims, t=0.74):
    """A dimmed copy of a prim list (fades every fill/stroke colour)."""
    out = []
    for p in prims:
        q = dict(p)
        if q.get("fill"):
            q["fill"] = _fade(q["fill"], t)
        if q.get("stroke"):
            q["stroke"] = _fade(q["stroke"], t)
        out.append(q)
    return out
...
if HAS_TK:
    ...
    class App(tk.Tk):
        ...
        def __init__(self):
            super().__init__()
            self.title('Flat "Strand"ley  —  topology sketchpad  ·  highlight/dim selection')
            ...
            self.redo_stack = []          # snapshots undone, available to redo
            self.clipboard = None         # copied chains (list of dicts)
            self.clip_cross = []          # internal crossing tops of the copy
            self._paste_n = 0
            self.cur_file = None
            self.dirty = False
            ...
            self.ui_scale = 1.0
            self.sc_font = tkfont.Font(family="Helvetica", size=8)  # shortcut hints
            self._base_fonts = {"__sc__": 8}
            for nm in ("TkDefaultFont", "TkTextFont", "TkHeadingFont",
                       "TkMenuFont", "TkSmallCaptionFont"):
                try:
                    self._base_fonts[nm] = tkfont.nametofont(nm).cget("size")
                except Exception:
                    pass
            ...
            self.bind_all("<Control-V>", lambda e: self.paste_selection())
            self.bind_all("<Control-g>", lambda e: self.create_group())
            self.bind_all("<Control-G>", lambda e: self.split_group())  # Ctrl+Shift+G
            ...
            self.bind_all("<Control-e>", lambda e: self.export_svg())
            self.bind_all("<Escape>", lambda e: self._escape())
            self.bind_all("<Up>", lambda e: self._nudge(0, -1))
            self.bind_all("<Down>", lambda e: self._nudge(0, 1))
            self.bind_all("<Left>", lambda e: self._nudge(-1, 0))
            self.bind_all("<Right>", lambda e: self._nudge(1, 0))
            self.bind_all("<Shift-Up>", lambda e: self._nudge(0, -5))
            self.bind_all("<Shift-Down>", lambda e: self._nudge(0, 5))
            self.bind_all("<Shift-Left>", lambda e: self._nudge(-5, 0))
            self.bind_all("<Shift-Right>", lambda e: self._nudge(5, 0))
        ...
        def _build_style(self):
            ...
            st.configure("Toolbar.TLabel", background="#ffffff", foreground="#888")
            st.configure("Hint.TLabel", background="#fafafb", foreground="#888")
        ...
        def _build_ui(self):
            ...
            ttk.Button(bottom, text="A+", width=3,
                       command=lambda: self._apply_ui_scale(0.15)).pack(side="right", padx=(0, 8))
            ttk.Button(bottom, text="A−", width=3,
                       command=lambda: self._apply_ui_scale(-0.15)).pack(side="right")
            ttk.Label(bottom, text="Panel / text size").pack(side="right", padx=(8, 4))
            self._set_status()

        def _topbar(self, bar):
            def b(t, c, sc=""):
                fr = ttk.Frame(bar, style="Toolbar.TFrame")
                fr.pack(side="left", padx=2, pady=2)
                ttk.Button(fr, text=t, command=c).pack()
                if sc:
                    ttk.Label(fr, text=sc, font=self.sc_font,
                              style="Toolbar.TLabel").pack()
            b("New", self.new_project, "Ctrl+N")
            b("Open", self.open_project, "Ctrl+O")
            b("Close", self.close_file, "Ctrl+W")
            b("Save", self.save_project, "Ctrl+S")
            b("Save As", self.save_project_as, "Ctrl+Shift+S")
            b("Export SVG", self.export_svg, "Ctrl+E")
            ttk.Separator(bar, orient="vertical").pack(side="left", fill="y", padx=6, pady=4)
            b("↶ Undo", self.undo, "Ctrl+Z")
            b("↷ Redo", self.redo, "Ctrl+Y")
            ttk.Separator(bar, orient="vertical").pack(side="left", fill="y", padx=6, pady=4)
        ...
        def _tools_panel(self, p):
            def tool_row(t, sc=""):
                ...
                fr.pack(anchor="w", fill="x", padx=8)
                ttk.Radiobutton(fr, text=self.TOOL_LABELS[t], value=t,
                                variable=self.tool,
            ...
            def cmd_row(text, command, sc=""):
                fr = ttk.Frame(p, style="Panel.TFrame")
                fr.pack(fill="x", padx=8, pady=2)
                ttk.Button(fr, text=text,
                           command=command).pack(side="left", fill="x", expand=True)
                if sc:
                    ttk.Label(fr, text=sc, font=self.sc_font,
                              style="Hint.TLabel").pack(side="right", padx=(4, 0))
            ...
            tool_hint = {"select": "Right-click", "selgroup": "Shift+Right-click"}
            for t in self.EDIT_TOOLS:
                tool_row(t, tool_hint.get(t, ""))
            ttk.Separator(p, orient="horizontal").pack(fill="x", pady=8, padx=6)
            cmd_row("Join selected ends", self.join_ends)
            cmd_row("Reverse whole chain", self.reverse_selected)
            cmd_row("Rename chain…", self.rename_chain)
            cmd_row("Delete selected", self.delete_selected)
            ttk.Separator(p, orient="horizontal").pack(fill="x", pady=8, padx=6)
            ttk.Label(p, text="Groups", style="Head.TLabel").pack(anchor="w", padx=8)
            cmd_row("Create Group…", self.create_group, "Ctrl+G")
            cmd_row("Add to group…", self.add_to_group)
            cmd_row("Split Group", self.split_group, "Ctrl+Shift+G")
            ttk.Separator(p, orient="horizontal").pack(fill="x", pady=8, padx=6)
            ttk.Label(p, text="Shortcuts", style="Head.TLabel").pack(anchor="w", padx=8)
            shortcuts = [
                ("Right-click", "select chain"),
                ("Shift+Right-click", "select group"),
                ("Drag (select tool)", "move selection"),
                ("Shift-click", "multi-select"),
                ("Arrow keys", "nudge 1 cell"),
                ("Shift+Arrows", "nudge 5 cells"),
                ("Middle-drag", "pan"),
                ("Mouse wheel", "zoom"),
            ...
            txt = "\n".join("%s — %s" % (k, v) for k, v in shortcuts)
            ttk.Label(p, text=txt, font=self.sc_font, style="Hint.TLabel",
        ...
        def _sel_is_group(self):
            """True when the current selection is exactly the full membership of
            one group (so Move treats it as a single group unit)."""
            if not self.sel:
                return False
            groups = {s.group for s in self._sel_strands()}
            if len(groups) == 1 and None not in groups:
                g = next(iter(groups))
                members = {s.id for s in self.strands if s.group == g}
                return members == set(self.sel)
            return False
        ...
        # ---- copy / paste ----------------------------------------
        def _focus_is_text(self):
            """True when a text field has focus, so Ctrl-C/V do text editing."""
            w = self.focus_get()
            return isinstance(w, (tk.Entry, tk.Spinbox, tk.Text))

        def _copy_name(self, base, used):
            cand = base + " (copy)"
            n = 2
            while cand in used:
                cand = "%s (copy %d)" % (base, n)
                n += 1
            return cand

        def _copy_group_name(self, base, used):
            cand = base + " (copy)"
            n = 2
            while cand in used:
                cand = "%s (copy %d)" % (base, n)
        ...
        def copy_selection(self):
            ...
            if not self.sel:
                ...
                return
            copied = self._sel_strands()
            self.clipboard = [s.to_dict() for s in copied]
            # remember every crossing internal to the copied set: its full
            # participant list plus which (strand, vertex) is on top, so the
            # paste reproduces the exact over/unders by identity.
            cross = compute_crossings(copied)
            tops = crossing_tops(cross, self.overrides)
            self.clip_cross = [([(s, v) for s, v, _ in lst], tops[p])
                               for p, lst in cross.items()]
            self._paste_n = 0
            ng = len({d["group"] for d in self.clipboard if d.get("group")})
            self._set_status("Copied %d chain(s)%s."
                             % (len(self.clipboard),
                                (" in %d group(s)" % ng) if ng else ""))
        ...
        def paste_selection(self):
            ...
            self.push_undo()
            self._paste_n += 1
            dx = dy = self._paste_n          # one cell down + right (per paste)
            used_names = {s.name for s in self.strands}
            used_groups = {s.group for s in self.strands if s.group}
            group_map = {}
            id_map = {}                      # old strand id -> new Strand
            new_ids = []
            for d in self.clipboard:
                s = Strand(d["path"])        # fresh id
                s.types = list(d["types"])
                s.colors = list(d["colors"])
                s.dirs = list(d.get("dirs", [1] * len(d["types"])))
                s.path = [(c + dx, r + dy) for c, r in s.path]
                s.name = self._copy_name(d.get("name", "Chain"), used_names)
                used_names.add(s.name)
                if d.get("group"):
                    g = d["group"]
                    if g not in group_map:
                        gn = self._copy_group_name(g, used_groups)
                        used_groups.add(gn)
                        group_map[g] = gn
                    s.group = group_map[g]
                else:
                ...
                new_ids.append(s.id)
                id_map[d["id"]] = s
            # reproduce the copy's internal over/unders (self-crossings and
            # crossings between strands of the copied group) by identity. Nothing
            # else is touched: the originals keep their own choices, and any
            # incidental overlap between the copy and other strands just defaults.
            for participants, top in self.clip_cross:
                if top[0] not in id_map:
                    continue
                key = frozenset((id_map[s].id, v) for s, v in participants if s in id_map)
                self.overrides[key] = (id_map[top[0]].id, top[1])
            self.sel = new_ids
            self.lin_sel = None
            self._update_sel_label()
            self.recompute()
            self._set_status("Pasted %d chain(s)." % len(new_ids))

        # ---- undo -------------------------------------------------
        def _snapshot(self):
            return {"strands": [s.to_dict() for s in self.strands],
                    "overrides": [[[list(e) for e in k], list(v)]
        ...
        def push_undo(self):
            ...
            self.redo_stack = []          # a fresh edit invalidates the redo trail
            self.dirty = True

        def _restore(self, snap):
            self.strands = [Strand.from_dict(d) for d in snap["strands"]]
            self.overrides = {frozenset(tuple(e) for e in k): tuple(v)
        ...
        def _nudge(self, dc, dr):
            """Arrow keys move the selected chain(s)/group by grid cells."""
            if self._focus_is_text() or not self.sel:
                return
            self.push_undo()
            for s in self._sel_strands():
                s.path = [(c + dc, r + dr) for c, r in s.path]
            self.recompute()

        def _escape(self):
            """Esc cancels an in-progress draw, otherwise clears the selection."""
            if self._focus_is_text():
                return
            if self.draw_path:
                self.draw_path = None
        ...
        def _recross(self):
            """Recompute crossings. Overrides are keyed by crossing identity
            (which strands cross), so they apply automatically wherever that same
            crossing exists -- no remapping needed when chains/groups move or a
            paste overlaps. Overrides are only dropped when a participating strand
            is deleted, so a crossing that briefly disappears (e.g. during an
            overlapping paste) restores its choice once it reappears."""
            self.crossings = compute_crossings(self.strands)
            live = {s.id for s in self.strands}
            self.overrides = {k: v for k, v in self.overrides.items()
                              if all(sid in live for sid, _ in k)}
        ...
        def render_main(self):
            ...
            prims = [(s, strand_prims(s.path, s.types, s.colors, s.dirs, self.cfg,
                                      ss_colors=self.ss_colors, color_mode=mode))
                     for s in self.strands]

            active = bool(self.sel) or bool(self.lin_sel)
            # whole strands to highlight: the selection, unless a linear stretch is
            # active (then nothing is fully highlighted -- only the stretch is)
            hi = set() if self.lin_sel else set(self.sel)

            if not active:
                for s, pr in prims:
                    c.draw_prims(pr)
                c.draw_prims(crossing_patches(self.strands, self.crossings,
                                              self.overrides, self.cfg,
                                              self.ss_colors, mode))
            else:
                lin_sid = self.lin_sel[0] if self.lin_sel else None
                a, b = (self.lin_sel[1], self.lin_sel[2]) if self.lin_sel else (0, 0)
                # bodies first: dim everything not highlighted; the stretch chain
                # is drawn in one masked pass so its joints / SS-ends stay correct
                for s, pr in prims:
                    if s.id not in hi and s.id != lin_sid:
                        c.draw_prims(_fade_prims(pr))
                for s, pr in prims:
                    if s.id in hi:
                        c.draw_prims(pr)
                if self.lin_sel:
                    ss = self._get(lin_sid)
                    if ss:
                        c.draw_prims(strand_prims(ss.path, ss.types, ss.colors,
                                                  ss.dirs, self.cfg,
                                                  ss_colors=self.ss_colors,
                                                  color_mode=mode, hi_range=(a, b)))
                # crossing patches LAST (over all bodies) so the over/under is
                # correct, fading each patch to match its top strand's dim state
                groups = crossing_patches(self.strands, self.crossings,
                                          self.overrides, self.cfg, self.ss_colors,
                                          mode, grouped=True)
                for sid, vidx, pr in groups:
                    full = (sid in hi) or (sid == lin_sid and a <= vidx < b)
                    c.draw_prims(pr if full else _fade_prims(pr))
                # selection overlays
                for s in self.strands:
                    if s.id in hi:
                        self._draw_selection(c, s)
                if self.lin_sel and self._get(lin_sid):
                    self._draw_subsection(c, self._get(lin_sid), a, b)
        ...
        def _draw_subsection(self, c, s, a, b):
            """Highlight just edges [a, b) of a chain (a linear-view stretch)."""
            sub = s.path[a:b + 1]
            pts = [c.w2s(px * UNIT, py * UNIT) for px, py in sub]
            if len(pts) >= 2:
                flat = [v for xy in pts for v in xy]
                c.create_line(*flat, fill="#0a84ff", width=4, dash=(3, 2))
            for (px, py) in (pts[0], pts[-1]):
                c.create_oval(px - 5, py - 5, px + 5, py + 5, outline="#0a84ff", width=2)

        def _begin_move(self, start, moving):
            """Arm a potential move; undo is pushed lazily on the first drag so a
            plain click (select) doesn't create an undo step."""
            self.move_ref = (start, [(m, [tuple(p) for p in self._get(m).path])
                                     for m in moving if self._get(m)])
            self._move_started = False

        def _apply_ui_scale(self, delta=0.0):
            """Scale the side-panel/linear-view fonts (and panel widths) so the
            interface can be made larger for readability."""
            import tkinter.font as tkfont
            self.ui_scale = max(0.7, min(2.2, self.ui_scale + delta))
            for nm, base in self._base_fonts.items():
                if nm == "__sc__":
                    continue
                try:
                    mag = max(7, int(round(abs(base) * self.ui_scale)))
                    tkfont.nametofont(nm).configure(size=(-mag if base < 0 else mag))
                except Exception:
                    pass
            sb = self._base_fonts.get("__sc__", 8)
            self.sc_font.configure(size=max(7, int(round(abs(sb) * self.ui_scale))))
            try:
                self.tools_panel.configure(width=int(150 * self.ui_scale))
                self.props_panel.configure(width=int(240 * self.ui_scale))
            except Exception:
                pass
            self.render_linear()
            self._set_status("Panel size %d%%" % round(self.ui_scale * 100))

        def _right_press(self, e):
            """Right-click selects the chain under the cursor and switches to the
            matching Select tool, so a following left-drag moves the selection;
            Shift+right-click selects its whole group."""
            sid = self._hit_strand(e.x, e.y, prefer=set(self.sel))
            if sid is None:
                return
            shift = bool(e.state & 0x0001)
            if shift:                                    # shift -> whole group
                s = self._get(sid)
                if s and s.group:
                ...
                self.sel = [sid]
            self.tool.set("selgroup" if shift else "select")
            self.lin_sel = None
            self._update_sel_label()
            self.render_main()
            self.render_linear()
        ...
        def _press(self, e):
            ...
            if t in ("helix", "sheet", "loop"):
                ...
                if shift:
                    if sid is not None:
                        if sid in self.sel:
                            ...
                            self.sel.append(sid)
                    # shift is a multi-select gesture, not a drag
                elif sid is None:
                    self.sel = []
                else:
                    if sid not in self.sel:
                        self.sel = [sid]
                    # else: keep the current selection so a drag moves all of it
                    moving = (list(self.sel)
                              if sid in self.sel and (len(self.sel) > 1
                                                      or self._sel_is_group())
                              else [sid])
                    self._begin_move(g, moving)
                self.lin_sel = None
                self._update_sel_label()
                self.render_main()
                self.render_linear()
                ...
                if sid is not None:
                    ...
                    self.lin_sel = None
                    self._update_sel_label()
                    self._begin_move(g, list(self.sel))
                    self.render_main()
                    self.render_linear()
                    self._set_status()
            elif t == "move":
                # prefer a chain already in the selection so a pasted group that
                # overlaps the originals isn't stolen by an underlying chain
                sid = self._hit_strand(e.x, e.y, prefer=set(self.sel))
                if sid is not None:
                    if sid in self.sel and (len(self.sel) > 1 or self._sel_is_group()):
                        moving = list(self.sel)        # move the whole group/selection
                    else:
                        self.sel = [sid]
                        moving = [sid]
                    self._update_sel_label()
        ...
        def _motion(self, e):
            ...
            if self.move_ref:
                ...
                if e.state & 0x0001:           # shift locks to the dominant axis
                    if abs(dc) >= abs(dr):
                        dr = 0
                    else:
                        dc = 0
                if (dc or dr) and not self._move_started:
                    self.push_undo()
        ...
        def _hit_strand(self, sx, sy, thresh=12, prefer=None):
            wx, wy = self.canvas.s2w(sx, sy)
            best, bestd = None, thresh / self.canvas.scale_
            pbest, pbestd = None, thresh / self.canvas.scale_
            for s in self.strands:
                for a, b in zip(s.path, s.path[1:]):
                    ...
                    if d < bestd:
                        bestd, best = d, s.id
                    if prefer and s.id in prefer and d < pbestd:
                        pbestd, pbest = d, s.id
            return pbest if pbest is not None else best
        ...
        def _toggle_crossing(self, sx, sy):
            ...
            entries = [(sid, vi) for sid, vi, _ in lst]
            key = cross_key(lst)
            cur = self.overrides.get(key, _default_top(lst))
            i = entries.index(cur) if cur in entries else 0
            self.push_undo()
            self.overrides[key] = entries[(i + 1) % len(entries)]
            self.render_main()
        ...
        def render_linear(self):
            c = self.lin
            c.delete("all")
            sc = getattr(self, "ui_scale", 1.0)
            rowh, cellw, labx = int(34 * sc), int(17 * sc), 8
            indent = int(30 * sc)             # deeper indent for grouped chains
            gfont = max(9, int(13 * sc))
            nfont = max(7, int(9 * sc))
            y = 10
            self._lin_layout = []
            ...
            for gi, (gname, members) in enumerate(sections):
                if gname is not None:
                    ...
                    c.create_text(nx, ny + 9, anchor="w", text=gname,
                                  font=("Helvetica", gfont, "bold"), fill="#0a6")
                ...
                for s in members:
                    ...
                    ne = len(s.types)
                    bx = labx + ind + int(70 * sc)
                    chain_sel = s.id in self.sel
                    stretch = self.lin_sel and self.lin_sel[0] == s.id
                    # whole-chain selection (made on the grid or in the linear list)
                    # shows as a tinted row here; a linear stretch stays distinct
                    if chain_sel:
                        rx0, ry0 = c.w2s(labx + ind - 4, y + 1)
                        rx1, ry1 = c.w2s(bx + ne * cellw + 6, y + rowh - 1)
                        c.create_rectangle(rx0, ry0, rx1, ry1,
                                           fill="#e8f2ff", outline="#9cc6ff", width=1)
                    x0, y0 = c.w2s(labx + ind, y)
                    namecol = "#0a6aff" if chain_sel else "#333"
                    c.create_text(x0, y0 + rowh / 2, anchor="w", text=s.name,
                                  font=("Helvetica", nfont, "bold"), fill=namecol)
                    for k in range(ne):
                        ...
                        cellsel = (self.lin_sel[1] <= k < self.lin_sel[2]) if stretch \
                            else chain_sel
                        c.create_rectangle(sx0, sy0, sx1, sy1, fill=col,
                                           outline="#0a84ff" if cellsel else "#333",
                                           width=2 if cellsel else 1)
                        midy = (sy0 + sy1) / 2
                        fwd = s.dirs[k] > 0
                        if s.types[k] == "sheet":
        ...
        def open_project(self):
            ...
            if isinstance(ov, list):                 # new identity-keyed format
                for k, v in ov:
                    self.overrides[frozenset(tuple(e) for e in k)] = tuple(v)
            else:                                    # legacy point-keyed: map via crossings
                pts = {}
                for key, val in ov.items():
                    cx, cy = key.split(",")
                    pts[(int(cx), int(cy))] = tuple(val)
                for p, lst in compute_crossings(self.strands).items():
                    if p in pts:
                        self.overrides[cross_key(lst)] = pts[p]
